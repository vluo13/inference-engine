...
export(t.detach().numpy(), 'tensors/tensor3d.bin')
export(F.gelu(t, approximate='tanh').detach().numpy(), 'tensors/tensor3d_gelu.bin')
...
for name, tt in [("neg", neg), ("zero", zero), ("large", large)]:
    ...
    export(tt.detach().numpy(), f'tensors/gelu_{name}_in.bin')
    export(out.detach().numpy(), f'tensors/gelu_{name}_out.bin')
...
for i, shape in enumerate(gelu_shapes):
    ...
    export(x.numpy(), f"tensors/gelu_rand_{i}_in.bin")
    export(out.numpy(), f'tensors/gelu_rand_{i}_out.bin')
...
for i, shape in enumerate(softmax_shapes):
    ...
    export(x.numpy(), f'tensors/softmax_rand_{i}_in.bin')
    export(out.numpy(), f'tensors/softmax_rand_{i}_out.bin')
...
for i, shape in enumerate(layernorm_shapes):
    ...
    export(x.numpy(), f'tensors/layernorm_rand_{i}_in.bin')
    export(w.numpy(), f'tensors/layernorm_rand_{i}_w.bin')
    export(b.numpy(), f'tensors/layernorm_rand_{i}_b.bin')
    export(out.numpy(), f'tensors/layernorm_rand_{i}_out.bin')
...
for i, (M, K, N) in enumerate(matmul_shapes):
    ...
    out = a @ b
    export(a.numpy(), f'tensors/matmul_rand_{i}_a.bin')
    export(b.numpy(), f'tensors/matmul_rand_{i}_b.bin')
    export(out.numpy(), f'tensors/matmul_rand_{i}_out.bin')
...
for i, (B, M, K, N) in enumerate(bmm_shapes):
    ...
    out = torch.bmm(a, b)
    export(a.numpy(), f'tensors/bmm_rand_{i}_a.bin')
    export(b.numpy(), f'tensors/bmm_rand_{i}_b.bin')
    export(out.numpy(), f'tensors/bmm_rand_{i}_out.bin')
